//06. Story page
var xmlDoc;
var title, participants, links, custom, route, summary, visa, exrate, habitation, transport, sights, souvenirs, food;

//06.01 Creator of page
function HTML_CreatorOfStoryPage(storyId) {
...
var processMyStory = function (data) {
    ...
    document.getElementById("someElement").innerHTML = HTML_StoryPage();
}
...
function routDateCalculator(date) {
    ...
    if (date.toString().length == 7) {
        ...
        month = russianMonth (date.toString().substring(1, 3)-1);
        year = date.toString().substring(3, 7);
    }
    else {
        day = date.toString().substring(0, 2);
        month = russianMonth(date.toString().substring(2, 4) - 1);
